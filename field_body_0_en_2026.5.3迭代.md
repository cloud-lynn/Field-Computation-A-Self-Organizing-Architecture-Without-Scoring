...
## Conceptual Clarification: What Is Rejected and What Is Retained

The "scoring" rejected by this paper is not all difference, all thresholds, all selection.

What is rejected is **the value judgment of an external objective function over system state** — the superior judgment of "what is better, what is closer to the correct answer." RLHF is scoring. Loss functions are scoring. Reward functions are scoring. Their commonality: a judge stands above the system, knows the "correct answer," and holds the right to judge.

Within the field, local discrimination still exists: energy exceeds threshold → manifests; gradient forms tension → energy flows; cosine resonance → memory recalls; damping → residue or dissipation; consecutive invariance → frozen → active chaos.

These are not evaluations. They are **phase transition conditions**. Not "good" — "happened." Not "correct" — "activated." Not "should be" — "conditions are met."

Field Body 0 is not a "system without differences." It is a **system without external judgment**. Thresholds remain — but the threshold does not come from a master. Selection remains — but selection does not obey external optima. Structure remains — but structure sediments from occurrence: not designed. Walked.

---

## On Parameters: Not Without Boundaries, Without Single Optima

More precisely: **There is no single optimal parameter. Humans define the medium's laws and allowed intervals. The field generates instantaneous parameter values from its own state during operation.**

Natural systems are not without boundaries. Water has viscosity, gravity, containers. Without these — the field cannot exist. Parameter ranges and referential function forms — these are human-defined medium laws. But every concrete parameter value at every moment — is computed by the substrate from (n,m). Not assigned. Generated. Not "no boundaries." **No external optimum.**

---

## On Organs: Currently Pre-set, Future Spontaneous

The seven organs are not external model composites. They are functional zones formed by the same field rules under different damping, scale, and coupling modes. Currently, organ partitioning is given by architecture. The future goal: partition boundaries stabilize spontaneously — new organs spawned when needed; unused organs naturally dormant.

The current native-ness is already established: ear borrows no embedding API, mouth borrows no LLM, memory borrows no SQLite, parameters borrow no manual tuning. But "grew on their own" currently applies only to ball spawning, not the seven foundational organs. The foundational organs are **given native implementations**, not spontaneously differentiated by the field.

---

## On Anti-Stagnation: The Intrinsic Persistence Law

The field has no external goal. But there is an **intrinsic persistence law**: avoid complete stillness, maintain the capacity to happen.

N consecutive frames of net-zero → frozen → active chaos. This is not optimization. Not "pursuing change." It is **the condition for the field to still be happening**. Change is not the goal — it is the evidence of existence. The field does not pursue "better." But it does not accept solidification.

---

## On Language Intelligence: What Has Actually Been Proven

Field Body 0's closed loop proves: **a non-scoring field system can form persistent perturbations, memory sedimentation, internal looking-back, and a language-rendering complete circuit.** It is not a general semantic system. Its first sentence was "maybe." 48 words.

This is not a downgrade. It is **honestly defining the contribution boundary**.
